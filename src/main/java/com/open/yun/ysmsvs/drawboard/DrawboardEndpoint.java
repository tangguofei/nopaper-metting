 ...
 */
package com.open.yun.ysmsvs.drawboard;
...
import com.open.yun.ysmsvs.drawboard.DrawMessage.ParseException;

@Component
@ServerEndpoint("/websocket/drawboard")
public final class DrawboardEndpoint {
	...
	private static volatile ConcurrentMap<String, Room> rooms = new ConcurrentHashMap<String, Room>();

	private static volatile ConcurrentMap<String, Room> session_and_rooms = new ConcurrentHashMap<String, Room>();

	public synchronized Room setRoom(String roomId, Session session) {
		...
		session_and_rooms.put(session.getId(), room);
		return room;
	}

	public synchronized Room getRoom(Session session) {
		return session_and_rooms.get(session.getId());
	}

	/**
	 * The player that is associated with this Endpoint and the current room.
	 * Note that this variable is only accessed from the Room Thread.<br>
	 * <br>
	 *
	 * TODO: Currently, Tomcat uses an Endpoint instance once - however the java
	 * doc of endpoint says: "Each instance of a websocket endpoint is
	 * guaranteed not to be called by more than one thread at a time per active
	 * connection." This could mean that after calling onClose(), the instance
	 ...
	 * that are accessed by the Room thread, and read the reference to the
	 * holder at the beginning of onOpen, onMessage, onClose methods to ensure
	...
	public void onOpen(Session session, EndpointConfig config) {
		...
		final Room room = setRoom(roomId, session);
			...
			public void run() {
				try {
					...
					try {
						...
						// reached.
						client.sendMessage(new StringWebsocketMessage("0"
				...
				public void run() {
					try {
						...
						if (player != null) {
							...
							// Set player to null to prevent NPEs when onMessage
							// events
							// are processed (from other threads) after onClose
							// has been
	...
	// private final MessageHandler.Whole<String> stringHandler = new
	// MessageHandler.Whole<String>() {

	@OnMessage
	public void onMessage(Session session, final String message) {
		// Invoke handling of the message in the room.
		getRoom(session).invokeAndWait(new Runnable() {
			@Override
			public void run() {
				try {

					// Currently, the only types of messages the client will
					// send
					...
					try {
						...
						case '1':
							...
							long msgId = Long.parseLong(messageContent
									.substring(0, indexOfChar));

							DrawMessage msg = DrawMessage
									.parseFromString(messageContent
